...
const RATES = {
    signature: [0.42, 0.41, 0.41, 0.4, 0.4, 0.39, 0.38, 0.37, 0.36, 0.35, 0.34, 0.33, 0.32, 0.31, 0.32, 0.31, 0.3, 0.29, 0.29, 0.29],
    transition: [0.8, 0.8, 0.77, 0.72, 0.68, 0.64, 0.61, 0.58, 0.55, 0.53, 0.51, 0.49, 0.47, 0.45, 0.43, 0.41, 0.39, 0.38, 0.35, 0.33],
...
let state = {
    ...
    frequency:  null,
    addons:     [],
    addonQuantities: { fridge: 0, oven: 0, windows: 0, org: 0 },
    basePrice:  0,
    totalPrice: 0,
    // Booking
    selectedDate: null,
...
function setupStep4() {
    ...
    setQtyAddonIncluded('addon-card-fridge', fridgeOvenIncluded, fridgeOvenLabel, '$60 per unit');
    setQtyAddonIncluded('addon-card-oven', fridgeOvenIncluded, fridgeOvenLabel, '$60 per unit');
...
function updatePrice() {
    ...
    // Add-ons — quantity counters (Fridge / Oven / Org / Windows)
    const counters = [
...
function renderPrice(base, maxDiscLabel, discountAmt, addonsTotal, total) {
    ...
    if (total > 0) {
        ...
        const bal = total - dep;
        html += `<div class="breakdown-item" style="margin-top:.25rem; font-size:0.85rem; color:var(--accent-primary);">
            <span>Required Deposit (50%)</span>
            <span>$${dep.toLocaleString()}</span>
        </div>
        <div class="breakdown-item" style="font-size:0.85rem; color:var(--text-secondary);">
            <span>Balance Due After Clean</span>
...
async function showBookingForm() {
    ...
    try {
        ...
        // This allows us to draw the beautiful ACH/Card interface immediately
        // without waiting for the backend server to respond!
        let renderAmount = Math.round(state.totalPrice * 100);
        if (renderAmount < 100) renderAmount = 10000; // Stripe requires minimum 100 cents.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Stripe (just the library)
    stripe = Stripe('pk_test_51TbqxWLpuu1PPzIHCW7zag9sDFj85kxANVJVKbJOpx9V7tuvvrnfzH6bUm1kvhr4l9jA4nCyPJATKknwuwXQ3Ggy00KQfBTjo3');
});
